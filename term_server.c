...
void * clnt_connection(void *arg)
{
   ...
   int i;
   while ((str_len = read(clnt_sock, s_data.message, sizeof(s_data))) != 0) {
      fflush(stdin);
      printf("[Client] %s\n", s_data.message);
      if (s_data.flag == 0) { //경고 모드(침입자 발생)
         if (strcmp(s_data.message, "Warning\n") == 0) {
            strcpy(sendmsg, "Warning message if you mistake, you enter this number /1457/\n");
            send_message(sendmsg, strlen(sendmsg));
            s_data.flag = 1; 
            write(clnt_sock, (char*)&s_data, sizeof(s_data));
         }
      }
      else if (s_data.flag == 1) { //보안코드 확인 모드
      ...
      else if(s_data.flag == 2){ //보안 카드가 입력됨 -> 보안 해제 
         strcpy(sendmsg, "Security card entered. Unsecure\n");
         send_message(sendmsg, strlen(sendmsg));
         s_data.flag = 2;
         write(clnt_sock, (char*)&s_data, sizeof(s_data));
      }
